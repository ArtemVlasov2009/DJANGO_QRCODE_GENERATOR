...
def render_history_gen(request):
    ...
    if request.user.is_authenticated:
        try:
            subscriber = Subscribers.objects.get(user=request.user)
            user_plan = subscriber.plan
        except Subscribers.DoesNotExist:
            pass

    if request.user.is_authenticated:
        qr_codes = qr_code.objects.filter(user=request.user, plan_created=user_plan)
    else:
        qr_codes = qr_code.objects.filter(plan_created='free')

    selected_plan = request.POST.get('selected_plan') or request.session.get('selected_plan')
    if selected_plan and request.user.is_authenticated:
        qr_codes = qr_code.objects.filter(user=request.user, plan_created=selected_plan)
        request.session['selected_plan'] = selected_plan
    current_date = timezone.now()

    for qr in qr_codes:
        naive_expiry_datetime = timezone.datetime.fromtimestamp(qr.expiry_date)
        expiry_datetime = timezone.make_aware(naive_expiry_datetime, timezone=timezone.utc)

        if current_date > expiry_datetime: 
            qr.is_expired = True
            qr.link = "/expired_error/"  
            qr.save()
        else:
            # Прострочен или нет
            qr.is_expired = True
...
def expired_error(request):
    return render(request, 'expired_error.html', {'message': 'Цей QR-код прострочений (старше 30 днів)'})
